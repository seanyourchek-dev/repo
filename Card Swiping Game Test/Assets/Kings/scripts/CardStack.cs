...
public class CardStack :  TranslatableContent {
    ...
    //software callbacks to inform scripts for changes
    public System.Action OnCardDestroy;
	...
	void Start(){
        ...
        resizeAllCardBlockCnt();

		//load the last card before quitting the game
		loadGameCard ();

        //load the follow up stack
        loadFollowUpStack();
	}
	...
	void addDrawCnt(GameObject go){
		...
		Debug.LogError ("Card was not found in stack. Card index couln't be saved." + go.name);
	}
	...
	IEnumerator CardMovement(){
        ...
        while (true) {
			if (anim != null && moveBackEnabled == true) {
				...
				if (swipe.pressed == false && swipe.idleConfig.idleActive == false) {
...
//spawn new card, dependent on priorities:
    ...
    #region follow up delay
    [System.Serializable]
    public class C_folluwUpStackElement
    {
        public cardIndex followUpCard;
        public int cardDelay;
        public int waitingTime;
    }

    [System.Serializable]
    public class C_WrapFollowUpStack
    {
        public List<C_folluwUpStackElement> followUpStack = new List<C_folluwUpStackElement>();
    }
    public C_WrapFollowUpStack followUpStack = new C_WrapFollowUpStack();
    ...
    void saveFollowUpStack()
    {
        string json = JsonUtility.ToJson(followUpStack);
        PlayerPrefs.SetString("followUpStack", json);
    }

    bool loadFollowUpStack()
    {
        string json = PlayerPrefs.GetString("followUpStack");
        if (string.IsNullOrEmpty(json))
        {
            return false;
        }
        ...
        {
            JsonUtility.FromJsonOverwrite(json, followUpStack);
            return true;
        }
    }
    ...
    public void decreaseFollowUpStackTime()
    {
		...
		for (int i = 0; i < allCards.Length; i++) {
			for (int j = 0; j < allCards [i].groupCards.Length; j++) {
				...
				if (es != null) {
                    ...
                    foreach (EventScript.eventText et in es.textFields.additionalTexts)
                    {
                        terms.Add(et.textContent);
                    }

                    terms.AddRange(es.Results.GetTranslatableContent());
                    foreach (EventScript.C_AdditionalModifiers am in es.changeExtrasOnCardDespawn)
                    {
                        terms.AddRange(am.GetTranslatableContent());
                    }
                }

                MakeChanges[] onCardAttachedChanges = allCards[i].groupCards[j].GetComponents<MakeChanges>();
                foreach(MakeChanges mc in onCardAttachedChanges)
                {
                    terms.AddRange(mc.GetTranslatableContent());
                }

                TextReplacementDisplay[] onCardTextReplacement = allCards[i].groupCards[j].GetComponents<TextReplacementDisplay>();
                foreach (TextReplacementDisplay tr in onCardTextReplacement)
                {
                    terms.AddRange(tr.getTranslatableTerms());
                }
            }
		}
